(() => {
    ...
    console.log("🏷️ Attributi Modal:", 
        Array.from(modal.attributes).map(a => `${a.name}="${a.value}"`).join(' | ') || "Nessun attributo"
    );
    ...
    if (parent) {
        ...
        console.log("🏷️ Attributi Padre:", 
            Array.from(parent.attributes).map(a => `${a.name}="${a.value}"`).join(' | ') || "Nessun attributo"
        );
    }
})();