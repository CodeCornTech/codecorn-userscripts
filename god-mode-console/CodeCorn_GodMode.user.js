// ==UserScript==
// @name         CodeCorn Console Capture (God Mode v4.13.3)
...
/* global html2canvas, GM_getValue, GM_setValue, GM_setClipboard, unsafeWindow */

// @ts-ignore
const _GM_getValue = typeof GM_getValue !== 'undefined' ? GM_getValue : null;
// @ts-ignore
const _GM_setValue = typeof GM_setValue !== 'undefined' ? GM_setValue : null;
// @ts-ignore
const _GM_setClipboard = typeof GM_setClipboard !== 'undefined' ? GM_setClipboard : null;
// @ts-ignore
const _unsafeWindow = typeof unsafeWindow !== 'undefined' ? unsafeWindow : null;
// @ts-ignore
const _html2canvas = typeof html2canvas !== 'undefined' ? html2canvas : null;

/**
 * @typedef {Window & typeof globalThis & { ccCopy?: function(any): any, html2canvas?: any, cccg?: any }} TargetWindow
 */
...
(() => {
    ...
    function ccLog(msg) {
         ...
         * @returns {string} 
         */
        const pad = (n) => { return n.toString().padStart(2, '0'); };
    ...
    function serializeValue(value, depth = 0, seen = new WeakSet()) {
        ...
        if (Array.isArray(value)) {
            return value.map((item) => { return serializeValue(item, depth + 1, seen); });
        }
    ...
    function pushEntry(method, args) {
        const entry = { ts: new Date().toISOString(), url: targetWindow.location.href, method, args: Array.from(args).map((arg) => { return serializeValue(arg); }) };
    ...
    targetWindow.XMLHttpRequest.prototype.send = function (...args) {
        if (userSettings.networkSpy) {
            ...
            this.addEventListener('load', () => { pushEntry('xhr (res)', [{ url: /** @type {any} */ (this)._cc_url, status: this.status }]); });
        }
    ...
    targetWindow.ccCopy = function (data) {
        ...
        try {
            ...
            navigator.clipboard
                .writeText(text)
                .then(() => { showToast(`✅ Dato copiato in clipboard!`); })
                .catch(() => {
    ...
    const getTs = () => {
        ...
        const p = (n) => { return n.toString().padStart(2, '0'); };
    ...
    function initMode(mode, msg) {
        ...
        let bg = 'rgba(14, 165, 233, 0.2)',
            border = '#0ea5e9'; // Default Inspector
        if (mode === 'thanos') {
    ...
    function handleHover(e) {
        ...
        if (target && (target.closest('#__cc_sidebar__') || target.closest('#__cc_backdrop__') || target === document.body || target === document.documentElement)) {
    ...
    const toggleInspector = () => { return currentMode === 'inspector' ? cleanupMode() : initMode('inspector', '🔍 Clicca per fotografare'); };
    const toggleThanos = () => { return currentMode === 'thanos' ? cleanupMode() : initMode('thanos', '💥 Seleziona per distruggere (Esc annulla)'); };
    const toggleSniffer = () => { return currentMode === 'sniffer' ? cleanupMode() : initMode('sniffer', '🎨 Seleziona elemento per copiare il CSS (Esc annulla)'); };
    const toggleMonitor = () => { return currentMode === 'monitor' ? cleanupMode() : initMode('monitor', '🎧 Seleziona elemento da monitorare (Esc annulla)'); };
    ...
    function toggleHelpModal() {
        ...
        code1.style.color = '#fbbf24'; code1.textContent = 'ccCopy(data)';
        api1.appendChild(code1); api1.appendChild(document.createTextNode(' - Copia oggetto/array in clipboard.'));
        
        const api2 = document.createElement('span');
        const code2 = document.createElement('code');
        code2.style.color = '#fbbf24'; code2.textContent = 'cccg';
        api2.appendChild(code2); api2.appendChild(document.createTextNode(" - Oggetto globale dell'estensione."));

        apiFlex.appendChild(api1); apiFlex.appendChild(api2);
        ...
        h3Vis.textContent = '⚙️ Impostazioni Visive';
        ...
        const createNumInput = (id, val, min, max, step) => {
            ...
            inp.type = 'number'; inp.id = id; inp.value = val.toString();
        ...
        openNodeModal('🌽 God Mode Settings', container, () => {
            /**
             * @param {string} id 
             * @param {string} key 
    ...
    function toggleInjectorModal() {
        ...
        const radCss = document.createElement('input'); radCss.type = 'radio'; radCss.name = 'cc_inj_type'; radCss.value = 'css'; radCss.checked = true;
        lblCss.appendChild(radCss); lblCss.appendChild(document.createTextNode(' CSS'));

        const lblJs = document.createElement('label');
        lblJs.style.cssText = `cursor:pointer; display:flex; align-items:center; gap:5px;`;
        const radJs = document.createElement('input'); radJs.type = 'radio'; radJs.name = 'cc_inj_type'; radJs.value = 'js';
        lblJs.appendChild(radJs); lblJs.appendChild(document.createTextNode(' JS'));

        radioWrap.appendChild(lblCss); radioWrap.appendChild(lblJs);
        ...
        openNodeModal('💉 Live Injector', container, () => {
    ...
    function togglePowerToolsModal() {
        ...
        const createPTBtn = (id, icon, text) => {
            ...
            b.id = id; b.style.cssText = btnCSS;
        ...
        openNodeModal('🛠️ Power Tools', container, () => {
            /**
             * @param {string} id 
    ...
    ccLog('God Mode v4.13.3 Inizializzato! 🚀 (Zero ESLint Errors / Trusted Types Compliant)');
})();
