...
(() => {
  ...
  const defaultSettings = {
    ...
    toolbarPosIdx: 0,
    toolbarVisible: true,
    iframePropagation: true, // Se true propaga X-Ray e tool negli iframe (es. customize-preview)
  };

  let userSettings = { ...defaultSettings };
  try {
    const savedRaw = _GM_getValue
      ? _GM_getValue(SETTINGS_KEY, null)
  ...
  /** @type {TargetWindow} */
  const targetWindow = _unsafeWindow
    ? /** @type {TargetWindow} */ (_unsafeWindow)
  ...
  const originalConsole = {
    ...
    debug: targetConsole.debug
      ? targetConsole.debug.bind(targetConsole)
      : targetConsole.log.bind(targetConsole),
  ...
  function loadHistoryLogs() {
    try {
      return (
        JSON.parse(targetWindow.localStorage.getItem(STORAGE_KEY) || "[]") || []
      );
    } catch {
      return [];
    }
  ...
  function saveHistoryLogs(logs) {
    try {
      targetWindow.localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify(logs.slice(-userSettings.maxLogs)),
      );
    } catch (e) {
  ...
  function serializeValue(value, depth = 0, seen = new WeakSet()) {
    ...
    if (
      ...
      value instanceof targetWindow.HTMLElement
    )
    ...
    /** @type {Object<string, any>} */
    const output = {};
    for (const key of isPlainObject(value)
      ? Object.keys(value)
  ...
  function pushEntry(method, args) {
    const entry = {
      ...
      method,
      args: Array.from(args).map((arg) => serializeValue(arg)),
    };
  ...
  // Intercetta i metodi base della console
  ["log", "info", "warn", "error", "debug"].forEach((method) => {
    ...
    if (consoleMethod) {
      ...
      const newFn = (...args) => {
        ...
        /** @type {Function} */ (
          /** @type {any} */ (originalConsole)[method]
  ...
  if (targetConsole.clear) {
    ...
    targetConsole.clear = (...args) => {
      ...
      if (originalConsole.clear)
        /** @type {Function} */ (originalConsole.clear).apply(
          originalConsole,
          args,
        );
    };
  ...
  targetWindow.fetch = async (...args) => {
    ...
    try {
      const res = await /** @type {Function} */ (origFetch).apply(
        targetWindow,
        args,
      );
      if (userSettings.networkSpy)
        pushEntry("fetch (res)", [{ url: res.url, status: res.status }]);
      return res;
    } catch (e) {
  ...
  targetWindow.XMLHttpRequest.prototype.send = function (...args) {
    if (userSettings.networkSpy) {
      ...
      this.addEventListener("load", () => {
        pushEntry("xhr (res)", [
          { url: /** @type {any} */ (this)._cc_url, status: this.status },
        ]);
      });
    }
  ...
  function cleanupMode() {
    ...
    tempOverlays.forEach((o) => o.remove());
    tempOverlays = [];
    selectedNodes = [];
    lcaNode = null;
  ...
  function initMode(mode, msg) {
    ...
    let bg = "rgba(14, 165, 233, 0.2)",
      border = "#0ea5e9"; // Inspector (Azzurro)
    if (mode === "thanos") {
    ...
    if (mode === "inspector")
      msg += " (Tieni premuto SHIFT per multi-selezione/area)";
    showToast(msg);
  ...
  function handleHover(e) {
    ...
    if (currentMode === "inspector" && selectedNodes.length > 0 && target) {
      ...
      targetToHighlight = /** @type {HTMLElement} */ (
        findLowestCommonAncestor(tempNodes) || target
      );
      isMulti = true;
    }
    ...
    if (targetToHighlight && overlay && tooltip) {
      ...
      appendSpan(
        tooltip,
        "span",
        targetToHighlight.tagName.toLowerCase(),
        "#f43f5e",
      );
      ...
      if (targetToHighlight.className) {
        appendSpan(
          tooltip,
          "span",
          `.${targetToHighlight.className.replace(/ /g, ".")}`,
          "#a3e635",
        );
      }
      ...
      if (isMulti) {
        ...
        appendSpan(
          tooltip,
          "span",
          `📦 Area Contenitore (Elementi: ${selectedNodes.length + 1})`,
          "#fbbf24",
        );
        tooltip.appendChild(document.createElement("br"));
        const hint = appendSpan(
          tooltip,
          "span",
    ...
    if (modeAtClick === "inspector") {
      if (e.shiftKey) {
        if (!selectedNodes.includes(target)) {
          ...
          showToast(
            `📍 Elemento aggiunto (${selectedNodes.length}). Continua o rilascia SHIFT.`,
          );
        }
        return;
      } else {
        ...
        if (selectedNodes.length > 0) {
          selectedNodes.push(target);
          finalTarget = /** @type {HTMLElement} */ (
            findLowestCommonAncestor(selectedNodes) || target
          );
        }
        ...
        try {
          ...
          downloadCanvas(
            canvas,
            selectedNodes.length > 0 ? "area_multipla" : "element",
          );
        } catch (err) {
    ...
    if (modeAtClick === "thanos") {
      ...
      ["click", "input", "change", "keydown", "focus", "blur"].forEach((ev) => {
        target.addEventListener(ev, (evt) => {
          ...
          ccLog(
            `🎧 Monitor [${ev}]: ${id} -> Valore: ${eventTarget.value || "N/A"}`,
          );
          pushEntry("event_monitor", [
            { event: ev, target: id, value: eventTarget.value },
          ]);
        });
      });
  ...
  const toggleInspector = () => {
    return currentMode === "inspector"
      ? cleanupMode()
      : initMode("inspector", "🔍 Clicca per fotografare");
  };
  const toggleThanos = () => {
    return currentMode === "thanos"
      ? cleanupMode()
      : initMode("thanos", "💥 Seleziona per distruggere (Esc annulla)");
  };
  const toggleSniffer = () => {
    return currentMode === "sniffer"
      ? cleanupMode()
      : initMode(
          "sniffer",
  ...
  const toggleMonitor = () => {
    return currentMode === "monitor"
      ...
      : initMode(
          "monitor",
          "🎧 Seleziona elemento da monitorare (Esc annulla)",
        );
  };
    ...
    if (hudActive) {
      ...
      function updateHUD() {
        ...
        if (now - hudLastTime >= 1000) {
          ...
          let ram = perf.memory
            ? (perf.memory.usedJSHeapSize / 1048576).toFixed(1) + " MB"
            : "N/A";
          if (hudEl) {
  ...
  function executeTrap() {
    Notification.requestPermission().then((perm) => {
      if (perm !== "granted") {
        showToast(
          "❌ Devi abilitare le notifiche per innescare la trappola!",
          true,
        );
        return;
      }

      // Ritardo randomico fra 2000ms e 6000ms per simulare comportamento umano
      const randomDelay = Math.floor(Math.random() * (6000 - 2000 + 1)) + 2000;
      showToast(
        `💣 Trappola innescata! Attesa stealth di ${(randomDelay / 1000).toFixed(1)}s prima dell'esplosione...`,
      );
      ...
      setTimeout(() => {
        const n = new Notification("⚠️ SISTEMA CRITICO", {
          ...
          requireInteraction: true, // Rende la notifica persistente
        });

        n.onclick = () => {
          // Magia 1: Focus sulla finestra
          window.focus();
          n.close();

          // Magia 2: Creiamo il modale ineluttabile
          const trapBd = createBackdrop();
          trapBd.style.background = "#000";
          trapBd.style.zIndex = "2147483647"; // Massimo possibile

          const modalBox = document.createElement("div");
          modalBox.style.cssText = `background:#1e293b; padding:40px; border-radius:12px; text-align:center; box-shadow:0 0 50px rgba(255,0,0,0.5); color:#fff; font-family:sans-serif; border: 2px solid red;`;

          modalBox.innerHTML = `
                        <h2 style="margin-top:0; font-size:28px;">ATTENZIONE</h2>
                        <p style="font-size:16px;">Confermi di voler procedere?</p>
                        <button id="__cc_trap_yes__" style="background:#ef4444; color:#fff; border:none; padding:15px 50px; font-size:22px; font-weight:bold; border-radius:8px; cursor:pointer; margin-top:20px; transition: all 0.2s; outline:none;">YES</button>
                    `;

          trapBd.appendChild(modalBox);
          document.body.appendChild(trapBd);

          // Traccia audio ignorante
          const audio = new Audio(
            "https://www.myinstants.com/media/sounds/epic.mp3",
          );

          // Animazione del bottone che "si clicca da solo"
          setTimeout(() => {
            const btn = document.getElementById("__cc_trap_yes__");
            if (btn) {
              ...
              setTimeout(() => {
                ...
                // Il play() passa liscio perché la User Gesture è ereditata dal click sulla notifica
                audio
                  .play()
                  .catch((e) => originalConsole.warn("Autoplay bloccato", e));

                setInterval(() => {
                  trapBd.style.background =
                    trapBd.style.background === "rgb(0, 0, 0)"
                      ? "rgb(255, 0, 0)"
                      : "rgb(0, 0, 0)";
                }, 200);
              }, 150);
  ...
  function moveToolbar() {
    if (!isSidebarVisible)
      return showToast(
        "⚠️ La toolbar deve essere visibile per spostarla!",
        true,
      );
  ...
  function toggleHelpModal() {
    ...
    visFlex.appendChild(
      createLabelRow(
        "Distanza bordo (px): ",
        createNumInput("cc_cfg_edge", userSettings.edgeOffset, 0, 100, null),
      ),
    );
    visFlex.appendChild(
      createLabelRow(
        "Toolbar Gap (px): ",
        createNumInput("cc_cfg_gap", userSettings.gap, 0, 30, null),
      ),
    );
    visFlex.appendChild(
      createLabelRow(
        "Auto-Hide Toolbar: ",
        createCbInput("cc_cfg_autohide", userSettings.autoHide),
      ),
    );

    const autoMsInp = createNumInput(
      "cc_cfg_automs",
      userSettings.autoHideMs,
      null,
      null,
    ...
    const showMsInp = createNumInput(
      "cc_cfg_showms",
      userSettings.autoShowMs,
      null,
      null,
    ...
    visFlex.appendChild(
      createLabelRow(
        "Hitbox (px): ",
        createNumInput("cc_cfg_zone", userSettings.hoverZone, null, null, 5),
    ...
    visFlex.appendChild(
      createLabelRow(
        ...
        createCbInput("cc_cfg_iframe", userSettings.iframePropagation),
        "Inietta i tool interattivi all'interno dei sotto-iframe",
      ),
    );
    ...
    sysFlex.appendChild(
      createLabelRow(
        ...
        createCbInput("cc_cfg_blockclear", userSettings.blockClear),
    ...
    sysFlex.appendChild(
      createLabelRow(
        ...
        createCbInput("cc_cfg_netspy", userSettings.networkSpy),
        "Logga richieste Fetch e XHR",
      ),
    );

    const maxLogInp = createNumInput(
      "cc_cfg_maxlogs",
      userSettings.maxLogs,
      null,
      null,
      500,
    );
    ...
    openNodeModal(
      ...
      () => {
        const ta = /** @type {HTMLTextAreaElement|null} */ (
          document.getElementById("__cc_inj_code__")
        );
        ...
        if (ta) {
          ta.addEventListener("keydown", function (e) {
            ...
            if (e.key === "Tab") {
              ...
              const start = el.selectionStart;
              const currentLine =
                el.value.substring(0, start).split("\n").pop() || "";
        ...
        if (btn) {
          ...
          btn.onclick = () => {
            const radio = /** @type {HTMLInputElement|null} */ (
              document.querySelector('input[name="cc_inj_type"]:checked')
            );
  ...
  function togglePowerToolsModal() {
    ...
    createPTBtn("pt_trap", "🤡", "Innesca Trappola (Stealth)");
    ...
    footer.textContent =
      "I Tool interattivi si chiudono cliccando su un elemento o premendo Esc.";
      ...
      () => {
        ...
        const bindPT = (id, hoverCol, action) => {
          ...
          if (b) {
            ...
            b.onclick = () => {
              action();
              if (["pt_sniffer", "pt_monitor", "pt_trap"].includes(id))
                closeModals();
            };
          }
        };
        ...
        bindPT("pt_trap", "#dc2626", executeTrap);
      },
      "400px",
    );
  }
  ...
  if (_GM_info && _GM_info.script) {
    ...
    if (headerVer !== targetWindow.cccg.version) {
      originalConsole.warn(
        `%c [CodeCorn] WARNING: Disallineamento Versione! %c Header TM: ${headerVer} | Core JS: ${targetWindow.cccg.version}`,
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    if (
      ...
      (window.location.pathname.includes("customize.php") &&
        !targetWindow.frameworkOverlayLoaded)
    ) {
    ...
    sidebar.appendChild(
      createBtn(
        "📜",
        "Full History (Ctrl+Shift+H)",
        "#0369a1",
        copyHistoryLogs,
      ),
    );
    sidebar.appendChild(
      createBtn("📦", "State Dump (Storage)", "#16a34a", exportStateDump),
    );
    sidebar.appendChild(
      createBtn(
        "📷",
    ...
    sidebar.appendChild(
      createBtn(
        "🎯",
        "Element Inspector (Ctrl+Shift+E)",
        "#7e22ce",
        toggleInspector,
      ),
    );
    sidebar.appendChild(
      createBtn("💥", "Thanos Snap", "#dc2626", toggleThanos),
    );
    sidebar.appendChild(
      createBtn("🩻", "X-Ray Layout (Ctrl+Shift+L)", "#475569", toggleXRay),
    );
    sidebar.appendChild(
      createBtn(
        "💉",
    ...
    sidebar.appendChild(
      createBtn(
        "⚙️",
        "Settings & Shortcuts (Ctrl+Shift+I)",
        "#0ea5e9",
        toggleHelpModal,
      ),
    );
  ...
  ccLog(
    "God Mode v4.15.0 Inizializzato! 🚀 (Trusted Types Compliant & WP Customizer Fixed)",
  );
})();
