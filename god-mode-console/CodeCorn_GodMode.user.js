...
/* @ts-ignore */
const _GM_info = typeof GM_info !== 'undefined' ? GM_info : null;
/* @ts-ignore */
...
(() => {
    ...
    const toggleThanos = () =>
        currentMode === 'thanos' ? cleanupMode() : initMode('thanos', '💥 Seleziona per distruggere (Esc annulla)');
    const toggleSniffer = () =>
    ...
    function executeTrap() {
        Notification.requestPermission().then((perm) => {
            if (perm !== 'granted') {
                showToast('❌ Devi abilitare le notifiche per innescare la trappola!', true);
                return;
            }
            ...
            setTimeout(() => {
                ...
                n.onclick = () => {
                    ...
                    // Magia 2: Creiamo il modale ineluttabile in modo Trusted Types safe!
                    ...
                    const trapTitle = document.createElement('h2');
                    trapTitle.style.cssText = 'margin-top:0; font-size:28px;';
                    trapTitle.textContent = 'ATTENZIONE';

                    const trapDesc = document.createElement('p');
                    trapDesc.style.cssText = 'font-size:16px;';
                    trapDesc.textContent = 'Confermi di voler procedere?';
                    ...
                    setTimeout(() => {
                        ...
                        if (btn) {
                            ...
                            setTimeout(() => {
                                ...
                                audio.play().catch((e) => originalConsole.warn('Autoplay bloccato', e));

                                setInterval(() => {
                                    trapBd.style.background =
                                        trapBd.style.background === 'rgb(0, 0, 0)' ? 'rgb(255, 0, 0)' : 'rgb(0, 0, 0)';
                                }, 200);
                            }, 150);
                        }
                    }, 800);
    ...
    function togglePowerToolsModal() {
        ...
        footer.textContent = 'I Tool interattivi si chiudono cliccando su un elemento o premendo Esc.';
            ...
            () => {
                ...
                const bindPT = (id, hoverCol, action) => {
                    ...
                    if (b) {
                        ...
                        b.onclick = () => {
                            action();
                            if (['pt_sniffer', 'pt_monitor', 'pt_trap'].includes(id)) closeModals();
                        };
                    }
                };
    ...
    // Se l'opzione iframePropagation è disattivata e non siamo nel top frame, blocchiamo l'esecuzione totale dello script qui.
    if (window !== window.top && !userSettings.iframePropagation) {
        return;
    }

    window.addEventListener('DOMContentLoaded', () => {
        // [FIX] Blocco tassativo del rendering UI negli iframe.
        // Se siamo in un iframe, fermiamo la costruzione della Sidebar per evitare conflitti con Turnstile e cloni grafici.
        // I logger e gli hook di rete configurati precedentemente continueranno a operare in background.
        ...
        /**
         * Genera un pulsante per la sidebar con Trusted Types DOM rendering.
         * @param {string} icon
         * @param {string} title
         * @param {string} hoverCol
        ...
        const createBtn = (icon, title, hoverCol, action) => {
            ...
            // Costruzione compatibile con Trusted Types
