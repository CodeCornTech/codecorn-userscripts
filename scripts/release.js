...
const filesToUpdate = [
    {
        ...
        replacements: [
            ...
            { regex: /(CodeCorn Console Capture \(God Mode v)\d+\.\d+\.\d+(\))/g, replace: `$1${newVersion}$2` },
            { regex: /(version:\s*["'])\d+\.\d+\.\d+(["'])/g, replace: `$1${newVersion}$2` },
            { regex: /(God Mode v)\d+\.\d+\.\d+( Inizializzato!)/g, replace: `$1${newVersion}$2` }
        ]
    },
    {
        name: 'index.html',
        replacements: [
            ...
            { regex: /(<span class="[^"]*text-emerald-400[^"]*">v)\d+\.\d+\.\d+(<\/span>)/g, replace: `$1${newVersion}$2` }
        ]
    },
    {
        name: 'README.md',
        replacements: [
            { regex: /(\|\s*\*\*God Mode Console Capture\*\*\s*\|\s*`v)\d+\.\d+\.\d+(`\s*\|)/g, replace: `$1${newVersion}$2` }
        ]
    },
    {
        name: 'package.json',
