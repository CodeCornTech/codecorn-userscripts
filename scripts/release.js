...
if (!newVersion || !/^\d+\.\d+\.\d+/.test(newVersion)) {
  console.error(
    "❌ Errore: Inserisci una versione valida (es: npm run release 4.15.0)",
  );
  process.exit(1);
}
...
const filesToUpdate = [
  ...
  },
  {
    ...
    replacements: [
      ...
      {
        regex:
          /(<span class="[^"]*text-emerald-400[^"]*">v)\d+\.\d+\.\d+(<\/span>)/g,
        replace: `$1${newVersion}$2`,
      },
    ],
  },
  {
    ...
    replacements: [
      {
        regex:
          /(\|\s*\*\*God Mode Console Capture\*\*\s*\|\s*`v)\d+\.\d+\.\d+(`\s*\|)/g,
        replace: `$1${newVersion}$2`,
      },
    ],
  },
...
filesToUpdate.forEach((fileDef) => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (content !== updated) {
      ...
      console.log(
        `⚠️ Nessun match trovato in: ${fileDef.name} (forse già aggiornato?)`,
      );
    }
  } else {
